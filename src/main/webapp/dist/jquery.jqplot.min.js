...
(function(H) {
	...
	H.jqplot.use_excanvas = (H.browser.msie && !H.jqplot.support_canvas()) ? true
			: false;
	...
	O.prototype.toggleDisplay = function(ac, ae) {
		...
		if (ad) {
			if (ab.canvas._elem.is(":hidden") || !ab.show) {
				...
				ab.canvas._elem.nextAll(
						".jqplot-point-label.jqplot-series-" + ab.index)
						.fadeIn(ad)
			} else {
				ab.show = false;
			...
			if (ab.canvas._elem.is(":hidden") || !ab.show) {
				...
				ab.canvas._elem.nextAll(
						".jqplot-point-label.jqplot-series-" + ab.index).show()
			} else {
				ab.show = false;
				ab.canvas._elem.addClass("jqplot-series-hidden");
				if (ab.shadowCanvas._elem) {
				...
				ab.canvas._elem.nextAll(
						".jqplot-point-label.jqplot-series-" + ab.index).hide()
			}
		}
	};
	function I() {
	...
	function N() {
		...
		this.parseOptions = function(ar) {
			...
			this.defaultAxisStart = (ar && ar.defaultAxisStart != null) ? ar.defaultAxisStart
		...
		function ac(ao) {
			...
			var ak = [ "xaxis", "yaxis", "x2axis", "y2axis", "y3axis",
					"y4axis", "y5axis", "y6axis", "y7axis", "y8axis", "y9axis",
					"yMidAxis" ];
			var af = am.axes;
			var ag, ah;
	...
	H.jqplot.CanvasGridRenderer.prototype.draw = function() {
		...
		for (var aq = 4; aq > 0; aq--) {
			...
			if (ab.show) {
				if (ab.drawBaseline) {
					...
					switch (aw) {
					case "xaxis":
						ai(this._left, this._bottom, this._right, this._bottom,
								av);
						break;
					case "yaxis":
					...
					case "x2axis":
						...
						break;
					case "y2axis":
						ai(this._right, this._bottom, this._right, this._top,
								av);
						break
					}
				}
		...
		if (this.shadow) {
			var ao = [ [ this._left, this._bottom ],
	...
	H.jqplot.LineRenderer.prototype.init = function(ac, ah) {
		...
		if (!this.isTrendline && this.fill || this.renderer.bands.show) {
			...
			if (!this.highlightColor) {
				var ae = (this.renderer.bands.show) ? this.renderer.bands.fillColor
						: this.fillColor;
			...
			for (var aw = 0; aw < 2; aw++) {
				...
				if (ay == 0 || ay == ab) {
					...
					if (ap[ay + 1] - ap[ay] == 0 || ap[ay] - ap[ay - 1] == 0) {
						...
						if (((ae[ay + 1] - ae[ay]) / (ap[ay + 1] - ap[ay]) + (ae[ay] - ae[ay - 1])
								/ (ap[ay] - ap[ay - 1])) == 0) {
							ac[aw] = 0
						} else {
							if ((ap[ay + 1] - ap[ay]) * (ap[ay] - ap[ay - 1]) < 0) {
								...
								ac[aw] = 2 / (ao(ae[ay + 1], ae[ay])
										/ (ap[ay + 1] - ap[ay]) + ao(ae[ay],
										ae[ay - 1])
										/ (ap[ay] - ap[ay - 1]))
							}
						}
		...
		for (var ah = 0, ad = ag.length; ah < ad; ah++) {
			if (ag[ah][0] != null && ag[ah][1] != null) {
				...
				if (ag[ah][0] == null) {
					ac = true;
					this.gridData
							.push([ null, ab.call(this._yaxis, ag[ah][1]) ])
				} else {
					if (ag[ah][1] == null) {
						ac = true;
						this.gridData.push([ af.call(this._xaxis, ag[ah][0]),
	...
	H.jqplot.LineRenderer.prototype.draw = function(aq, aC, ac, av) {
		...
		var ab = (ak.fillAndStroke != r) ? ak.fillAndStroke
				: this.fillAndStroke;
		var al, ar, ao, ay;
		aq.save();
		if (aC.length) {
				...
				if (au) {
					if (this.fillToZero) {
						...
						if (this.index == 0 || !this._stack) {
							var ap = [];
							var aF = (this.renderer.smooth) ? this.renderer._smoothedPlotData
									: this._plotData;
							this._areaPoints = [];
							...
							if (this.fillAxis == "y") {
								...
								this._areaPoints.push([ aC[aC.length - 1][0],
										aA ])
							}
							if (ae) {
					...
					if (this.renderer.bands.show) {
						...
						if (this.renderer.bands.showLines) {
							ag = (this.renderer.smooth) ? this.renderer._hiBandSmoothedData
									: this.renderer._hiBandGridData;
							this.renderer.shapeRenderer.draw(aq, ag, ak);
							ag = (this.renderer.smooth) ? this.renderer._lowBandSmoothedData
									: this.renderer._lowBandGridData;
	...
	H.jqplot.LinearAxisRenderer.prototype.createTicks = function(ad) {
		...
		var ab = (this.name.charAt(0) === "x") ? this._plotDimensions.width
				: this._plotDimensions.height;
		var ah;
		var a0, aC;
		var aj, ai;
			...
			if (this.min == null || this.max == null
				...
				if (this.autoscale && this.min == null && this.max == null) {
					...
					for (var aU = 0; aU < this._series.length; aU++) {
						...
						var ax = (aP.fillAxis == "x") ? aP._xaxis.name
								: aP._yaxis.name;
						if (this.name == ax) {
						...
						if (aH) {
							...
							if (this.numberTicks == null) {
								if (this.tickInterval) {
									this.numberTicks = 3 + Math.ceil(ao
											/ this.tickInterval)
								} else {
									this.numberTicks = 2 + Math
											.ceil((ab - (this.tickSpacing - 1))
					...
					if (aj.show && aj.showLabel) {
						...
						if (aj.constructor == H.jqplot.CanvasAxisTickRenderer
							...
							case "auto":
								...
								break;
							case "end":
								ab = -aj.getWidth() + aj._textRenderer.height
										* Math.sin(-aj._textRenderer.angle) / 2;
								break;
							case "start":
								ab = -aj._textRenderer.height
										* Math.sin(aj._textRenderer.angle) / 2;
	...
	H.jqplot.MarkerRenderer.prototype.drawDiamond = function(ad, ac, ag, af, ai) {
		...
		var ae = [ [ ad - aj, ac ], [ ad, ac + ah ], [ ad + aj, ac ],
				[ ad, ac - ah ] ];
		if (this.shadow) {
	...
	H.jqplot.MarkerRenderer.prototype.drawSquare = function(ad, ac, ag, af, ai) {
		...
		var aj = this.size / 2 / ab;
		var ah = this.size / 2 * ab;
		var ae = [ [ ad - aj, ac - ah ], [ ad - aj, ac + ah ],
				[ ad + aj, ac + ah ], [ ad + aj, ac - ah ] ];
		if (this.shadow) {
			this.shadowRenderer.draw(ag, ae)
		}
		this.shapeRenderer.draw(ag, ae, ai)
		...
		if (!this.themes.hasOwnProperty(au)) {
			...
			for ( var ar in ao.axes) {
				...
				if (ae.show) {
					...
					at = (ag.axesStyles.borderColor != null) ? ag.axesStyles.borderColor
							: af.borderColor;
					if (af.borderColor != null) {
						ae.borderColor = af.borderColor;
						ab = true
					}
					at = (ag.axesStyles.borderWidth != null) ? ag.axesStyles.borderWidth
							: af.borderWidth;
			...
			for (ap = 0; ap < ag.series.length; ap++) {
				...
				for (an in ag.series[ap]) {
					at = (ag.seriesStyles[an] != null) ? ag.seriesStyles[an]
							: ag.series[ap][an];
					if (at != null) {
						aj[an] = at;
						if (an == "color") {
	...
	H.fn.jqplotGetComputedFontStyle = function() {
		var ae = window.getComputedStyle ? window.getComputedStyle(this[0], "")
				: this[0].currentStyle;
	...
	H.fn.jqplotToImageCanvas = function(ad) {
		...
		var af = (ad.backgroundColor == null) ? "rgb(255,255,255)"
				: ad.backgroundColor;
		...
		function ap(aA, aD, ay) {
			...
			var aE = window.getComputedStyle ? window.getComputedStyle(aA, "")
					: aA.currentStyle;
			...
			if ((aH == "div" || aH == "span")
				...
				if (aH === "table" && H(aA).hasClass("jqplot-table-legend")) {
							...
							.each(
									function() {
										...
										ae.strokeRect(aK, aL, aO.innerWidth(),
												aO.innerHeight());
										aK += parseInt(aO.css("padding-left"),
												10);
										aL += parseInt(aO.css("padding-top"),
												10);
										var aN = aO.innerHeight()
												- 2
												* parseInt(aO
														.css("padding-top"), 10);
										var aJ = aO.innerWidth()
												- 2
												* parseInt(aO
														.css("padding-left"),
														10);
					...
					H(aA).find("td.jqplot-table-legend-label").each(
							function() {
								var aL = H(this);
	...
	aa.matchers = [
			...
			function(ae) {
				...
				if (ac) {
					...
					if (ac[6]) {
						ab = ac[6].toLowerCase() == "am" ? (ab == 12 ? 0 : ab)
								: (ab == 12 ? 12 : ab + 12)
					}
					ad.setHours(ab, parseInt(ac[3] || 0, 10), parseInt(
							ac[4] || 0, 10),
		...
		return ag
				.replace(
						...
						function(aG, aq, ar, av, aI, aD, ao) {
							...
							case "c":
								return ab(String.fromCharCode(+az), ax, av, aD,
										au, ap);
							case "b":
							...
							case "i":
								...
								var aB = al < 0 ? "-" : at;
								var aE = an ? ae(String(Math.abs(al)))
										: String(Math.abs(al));
							...
							case "G":
								...
								var aB = al < 0 ? "-" : at;
								var am = [ "toExponential", "toFixed",
										"toPrecision" ]["efg".indexOf(ao
										.toLowerCase())];
								var aH = [ "toString", "toUpperCase" ]["eEfFgG"
										.indexOf(ao) % 2];
							...
							case "P":
								...
								var aB = al < 0 ? "-" : at;
								var ay = String(
										Number(Math.abs(al)).toExponential())
										.split(/e|E/);
								var ak = (ay[0].indexOf(".") != -1) ? ay[0].length - 1
										: ay[0].length;
								...
								var aH = [ "toString", "toUpperCase" ]["pP"
										.indexOf(ao) % 2];
...
function _normalizeArguments(b, a, c, d) {
	...
	c = c || a.duration;
	b.duration = $.fx.off ? 0 : typeof c === "number" ? c
			: c in $.fx.speeds ? $.fx.speeds[c] : $.fx.speeds._default;
...
$.fn
		.extend({
			jqplotEffect : function(i, j, b, h) {
				...
				if (f) {
					return e === false ? this.each(c) : this
							.queue(e || "fx", c)
				} else {
					return a.call(this, {
						options : g,
...
$.jqplot.effects.effect.blind = function(c, h) {
	var d = $(this), k = [ "position", "top", "bottom", "left", "right",
			...
			|| "hide"), m = c.direction || "up", f = rvertical.test(m), e = f ? "height"
